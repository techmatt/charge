...
ID3DBlob* D3D11Utility::CompileShader(const string &filename, const string &entryPoint, const string &shaderModel, const vector<pair<string, string>>& macros)
{
	...
	copy(entryPoint.begin(), entryPoint.end(), compiledFilename.begin() + oldLen);
	...
	if (shader_macros) {
		...
		for (unsigned int i = 0; shader_macros[i].Name != NULL; i++) {
			string name(shader_macros[i].Name);
			if (name[0] == '\"')				name[0] = 'x';
			if (name[name.length() - 1] == '\"')	name[name.length() - 1] = 'x';
			string def(shader_macros[i].Definition);
			if (def[0] == '\"')				def[0] = 'x';
			...
			copy(name.begin(), name.end(), compiledFilename.begin() + oldLen);
			copy(def.begin(), def.end(), compiledFilename.begin() + name.length() + oldLen);
		}
	}
	compiledFilename.push_back('.');
	...
	hFindShader = FindFirstFileA(string(filename.begin(), filename.end()).c_str(), &findData_shader);
...
//#ifdef UNICODE
		wstring s(filename.begin(), filename.end());
//#else
...
//#endif
		...
		if (FAILED(hr)) {
			...
			if (errorBlob != nullptr) {
				...
				cout << "Shader compilation failed for " << filename << endl
					<< errorBlobText << endl;
			}
		...
		if (s_bUsePreCompiledShaders) {
			ofstream compiledFile(compiledFilename.c_str(), ios::out | ios::binary);
