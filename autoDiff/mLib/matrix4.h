...
*/
template <class FloatType> class Matrix4
{
public:
	...
	Matrix4(const FloatType* values) {
	...
	Matrix4(const vec3<FloatType> &v0, const vec3<FloatType> &v1, const vec3<FloatType> &v2) {
	...
	Matrix4(const vec3<FloatType> &v0, const vec3<FloatType> &v1, const vec3<FloatType> &v2, const vec3<FloatType> &v3) {
	...
	Matrix4(const vec4<FloatType> &v0, const vec4<FloatType> &v1, const vec4<FloatType> &v2, const vec4<FloatType> &v3) {
		matrix2[0][0] = v0.x;	matrix2[0][1] = v0.y;	matrix2[0][2] = v0.z;	matrix2[0][3] = v0.w;
		matrix2[1][0] = v1.x;	matrix2[1][1] = v1.y;	matrix2[1][2] = v1.z;	matrix2[1][3] = v1.w;
		matrix2[2][0] = v2.x;	matrix2[2][1] = v2.y;	matrix2[2][2] = v2.z;	matrix2[2][3] = v2.w;
	...
	Matrix4(	const FloatType &m00, const FloatType &m01, const FloatType &m02, const FloatType &m03,
	...
	static Matrix4 zero(FloatType v = (FloatType)0) {
		Matrix4 res;	res.setZero(v);
		return res;
	}
	...
	static Matrix4 translation(FloatType x, FloatType y, FloatType z) {
		Matrix4 res;	res.setTranslation(x,y,z);
		return res;
	}
	...
	static Matrix4 translation(const vec3<FloatType>& v) {
		Matrix4 res;	res.setTranslation(v);
		return res;
	}
	...
	static Matrix4 rotationX(FloatType angle) {
		Matrix4 res;	res.setRotationX(angle);
		return res;
	}
	...
	static Matrix4 rotationY(FloatType angle) {
		Matrix4 res;	res.setRotationY(angle);
		return res;
	}
	...
	static Matrix4 rotationZ(FloatType angle) {
		Matrix4 res;	res.setRotationZ(angle);
		return res;
	}
	...
	static Matrix4 rotation(FloatType yaw, FloatType pitch, FloatType roll) {
		Matrix4 res;	res.setRotation(yaw, pitch, roll);
		return res;
	}
	...
	static Matrix4 rotation(const vec3<FloatType> &axis, FloatType angle, const vec3<FloatType>& center) {
		Matrix4 ret;
	...
	static Matrix4 scale(FloatType x, FloatType y, FloatType z) {
		Matrix4 res;	res.setScale(x,y,z);
		return res;
	}
	...
	static Matrix4 scale(FloatType s) {
		Matrix4 res;	res.setScale(s);
		return res;
	}
	...
	static Matrix4 scale(const vec3<FloatType>& v) {
		Matrix4 res;	res.setScale(v);
		return res;
	}

    static Matrix4 face(const vec3<FloatType>& vA, const vec3<FloatType>& vB)
    {
	...
	static Matrix4 diag(FloatType x, FloatType y, FloatType z, FloatType w) {
		Matrix4 res;	res.setDiag(x,y,z,w);
		return res;
	}
	...
	Matrix4& operator*= (const Matrix4& other) {
		Matrix4<FloatType> prod = (*this) * other;
		*this = prod;
		return *this;
	}

	//! multiply each element in the matrix with a scalar factor
	Matrix4 operator* (FloatType r) const {
		Matrix4<FloatType> result;
		for (unsigned int i = 0; i < 16; i++) {
    ...
    Matrix4<FloatType> getInverse() const {
		FloatType inv[16];
	...
	unsigned int rank(FloatType eps = (FloatType)0.00001) const {
		return util::rank<Matrix4<FloatType>, FloatType>(*this, 4, eps);
	}
  ...
  private:
	...
	////! quaternion is a friend, because it should be able to instance an uninitialized matrix (Matrix4 Quaternion::matrix())
	//friend class Quaternion<FloatType>;
};

//! writes to a stream
template <class FloatType> 
inline std::ostream& operator<<(std::ostream& s, const Matrix4<FloatType>& m)
{ 
	return (
		s << 
...
//! reads from a stream
template <class FloatType> 
inline std::istream& operator>>(std::istream& s, const Matrix4<FloatType>& m)
{ 
	return (
		s >> 
